...
describe('contentEditable utilities', () => {
  ...
  describe('markdownToHtml', () => {
    ...
    // 注意：编辑器不转换 --- 为 hr，保持为文本。时间线用 react-markdown 渲染 hr。
    it('should keep --- as text (not convert to hr)', () => {
  ...
  describe('htmlToMarkdown', () => {
    ...
    // 注意：hr 不在 ALLOWED_TAGS 中，会被 sanitize 移除
    it('should strip hr tags (not allowed in editor)', () => {
  ...
  describe('roundtrip conversion', () => {
    ...
    const testCases: Array<{ input: string; expected: string }> = [
      ...
      // Horizontal rule stays as text in editor
      { input: '---', expected: '---' },
    ]
