...
| 日期导航 | 支持前后切换和日期选择 | `<` `>` 按钮可用，点击日期弹出选择器，有日记的日期显示圆点标记 |
...
| 沉浸式输入 | 无边框输入区域，整个屏幕可输入 | 输入区域无明显边框和分割线 |
| 底部工具栏 | 固定在底部，包含图片、加粗、列表按钮 | 键盘弹出时工具栏跟随上移 |
| 文本输入 | 支持多行文本，受限 Markdown | 支持加粗、斜体、有序/无序列表 |
| 图片上传 | 最多 3 张，自动压缩 | 通过工具栏添加，图片展示在文字下方 |
| 保存 | 右上角保存按钮或键盘快捷键 | 保存后返回时间线对应日期 |
| 取消 | 左上角取消按钮或键盘快捷键 | 有修改时二次确认 |
| 删除 | 编辑模式底部删除按钮 | 二次确认后删除 |
| 键盘快捷键 | Cmd/Ctrl+Enter 保存，Esc 取消 | 与按钮行为一致 |
...
| 跳转时间线 | 点击结果跳转到对应日期的时间线 | 自动滚动到该条日记 |
